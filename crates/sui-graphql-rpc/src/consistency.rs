...
pub(crate) fn build_objects_query_v2(
    ...
    let history_objs = {
        ...
        if matches!(view, View::Consistent) {
            ...
            // Subquery to be used in `LEFT JOIN` against the inner querie for more recent object versions
            let newer = newer_criteria(filter!(
                query!("SELECT object_id, object_version FROM objects_history"),
                format!(
                    r#"checkpoint_sequence_number BETWEEN {} AND {}"#,
            ...
            history_objs_inner = filter!(history_objs_joined, "newer.object_version IS NULL");
        }
